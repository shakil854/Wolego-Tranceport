...
import { Printer, Download, Share2, Search, DollarSign } from 'lucide-react-native';
import HeaderBar from '../../components/HeaderBar';
import SearchableSelect from '../../components/SearchableSelect';
import { fetchPartiesApi, fetchLREntriesApi } from '../../api/endpoints';
...
export default function PartyStatementScreen({ navigation }) {
  ...
  const loadData = async () => {
    try {
      const [partyRes, lrRes] = await Promise.all([
        fetchPartiesApi(),
        fetchLREntriesApi(),
      ]);
      setParties(partyRes || []);
      ...
      if (partyRes && partyRes.length > 0) {
        setSelectedParty(partyRes[0]);
      }
    } catch (e) {
      console.error("Load statement data error:", e);
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredLrs = allLrs.filter((lr) => {
    ...
    const isMatchingParty =
      lr.consignorId === selectedParty.id ||
      lr.consigneeId === selectedParty.id ||
      String(lr.consignorName || '').toLowerCase() === String(selectedParty.partyName || '').toLowerCase() ||
      String(lr.consigneeName || '').toLowerCase() === String(selectedParty.partyName || '').toLowerCase();

    if (!isMatchingParty) return false;
    ...
    return `
      ...
      <body>
        <div class="header">
          ...
          <div style="font-weight:bold; margin-top:4px;">PARTY ACCOUNT & PAYMENT STATEMENT</div>
          <div>PARTY: <strong>${selectedParty?.partyName || 'ALL'}</strong> | PERIOD: ${selectedFY}</div>
        </div>
  ...
  const handleExportPDF = async () => {
    try {
      const html = generateStatementHtml();
      const filename = `Party_Statement_${selectedParty?.partyName || 'All'}.pdf`;
      const fileUri = await generateAndSavePdfNative(html, filename);
      await shareFileNative(fileUri, "Party Statement PDF");
    } catch (e) {
      Alert.alert("Error", "Failed to export statement PDF.");
    }
  };
  ...
  const handleShareWhatsApp = async () => {
    try {
      ...
      const filename = `Party_Statement_${selectedParty?.partyName || 'All'}.pdf`;
      const fileUri = await generateAndSavePdfNative(html, filename);
      const msg = `Wolego Transport Account Statement\nParty: ${selectedParty?.partyName}\nTotal Billed: ₹ ${formatCurrency(totalBilled)}\nTotal Paid: ₹ ${formatCurrency(totalPaid)}\nPending Balance: ₹ ${formatCurrency(totalRemaining)}`;
      await shareToWhatsApp(selectedParty?.mobileNos, msg, fileUri);
    } catch (e) {
  ...
  return (
    <View style={styles.container}>
      <HeaderBar title="Party Statement Ledger" />

      <View style={styles.topBar}>
        <TouchableOpacity style={styles.selectBtn} onPress={() => setPartyModal(true)}>
          <Text style={styles.selectText} numberOfLines={1}>
            Party: {selectedParty?.partyName || "Select Party"}
          </Text>
      ...
      {loading ? (
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          ...
          {/* Statement Table */}
          <Text style={styles.sectionTitle}>LR Transactions ({filteredLrs.length})</Text>
          {filteredLrs.map((lr) => (
...
const styles = StyleSheet.create({
  ...
  selectBtn: { backgroundColor: '#0f172a', borderRadius: 8, borderWidth: 1, borderColor: '#334155', paddingHorizontal: 12, height: 42, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 },
  ...
  btnBar: { flexDirection: 'row', gap: 8, marginBottom: 16 },
  ...
  waBtn: { flex: 1, height: 42, backgroundColor: '#16a34a', borderRadius: 8, justifyContent: 'center', alignItems: 'center', flexDirection: 'row', gap: 6 },
